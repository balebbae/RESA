...
export function ShiftCard({ shift, onClick, layoutProps }: ShiftCardProps) {
  ...
  // Use role color from shift (always present from backend, fallback for safety)
  const backgroundColor = shift.role_color;
  // Darken the role color for the border (reduce lightness by ~20%)
  const borderColor = darkenColor(shift.role_color);
