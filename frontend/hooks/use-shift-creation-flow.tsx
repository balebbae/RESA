...
export function useShiftCreationFlow({
  ...
  const {
    ...
    onError: (error) => {
      showErrorToast("Failed to modify shift", error.message);
    },
  });
      ...
      try {
        ...
        if (!updatedShift.start_time || !updatedShift.shift_date) {
          ...
          throw new Error(
            ...
            `start_time=${updatedShift.start_time} (${typeof updatedShift.start_time}), ` +
            `shift_date=${updatedShift.shift_date} (${typeof updatedShift.shift_date})`
          );
        }
